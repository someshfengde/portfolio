        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {profile.certifications.map((c) => (
            <StaggerItem key={`${c.name}-${c.issuer}`}>
              <li className="rounded-lg border border-foreground/10 p-5 hover:border-accent/20 hover:shadow-sm transition-all">
                <div className="text-sm font-semibold">{c.name}</div>
                <div className="text-xs text-foreground/60 mt-0.5">{c.issuer}</div>
